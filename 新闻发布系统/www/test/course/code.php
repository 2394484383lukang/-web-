...
for($d=0;$d<300;$d++){
	...
	imagesetpixel($img, mt_rand(1,80), mt_rand(1,30), $color);
}
//画线操作
for($x=0;$x<5;$x++){
	$color  = imagecolorallocate($img, mt_rand(0,255), mt_rand(0,255), mt_rand(0,255));
	imageline($img, 0, mt_rand(0,30), 80, mt_rand(0,30), $color);
}

for($j=0;$j<strlen($rnd_str);$j++){
	imagettftext($img, 16, mt_rand(0,10), $j*16+10, mt_rand(15,30), $fg, $fontfile, $rnd_str[$j]);
}
